...
public class ManageVelocity {
    public static boolean knockback(Player player, final double power, double xd, double zd, final DamageSource source, final float damage, final boolean comesFromEffect) {
        if (source.getEntity() == null || player.getMainHandItem().isEmpty())
            return false;
        
        var knockbackModifier = PlayerCubeUtil.knockbackModifiers(player);
        var soundSettings = PlayerCubeUtil.soundSettings(player);
        ...
        Holder<SoundEvent> hitSound = soundSettings.hitSound();

        Vec2 originalAngle = new Vec2((float)xd, (float)zd);
        Vec2 newAngle = applyHorizontalHitAngleScale(horizontalHitAngleScale, originalAngle, source.getEntity().getEyePosition(), source.getEntity().getLookAngle().normalize(),
            player.getBoundingBox().getCenter());
        Vec2 newPower = applyVerticalHitAnglePowerTransfer(verticalHitAngleScale, horizontalPower, verticalPower, source.getEntity().getEyePosition(), source.getEntity().getLookAngle().normalize(),
            player.getBoundingBox().getCenter(), player.getBbHeight());
        horizontalPower = newPower.x;
        verticalPower = newPower.y;
        newPower = applyVerticalPositionAnglePowerRotation(verticalPositionAngleScale, horizontalPower, verticalPower, originalHorizontalPower, originalVerticalPower, source.getEntity().position(),
            player.position());
        horizontalPower = newPower.x;
        verticalPower = newPower.y;
        xd = newAngle.x;
        ...
        Vec3 horizontalKnockback = (new Vec3(xd, 0.0F, zd)).normalize().scale(horizontalPower);
        player.setDeltaMovement(deltaMovement.x - horizontalKnockback.x, deltaMovement.y + (double)verticalPower * 1.2, deltaMovement.z - horizontalKnockback.z);

        PlayerCubeUtil.makeSound(player, hitSound.value());
        return true;
    }

    private static Vec2 applyHorizontalHitAngleScale(final float horizontalAngleScale, final Vec2 originalAngle, final Vec3 attackerPosition, final Vec3 attackerAimDirection,
                                                     final Vec3 targetCenter) {
        Vec3 attackerToTarget = targetCenter.subtract(attackerPosition).normalize();
        float angleDiff = (float) Mth.atan2(attackerAimDirection.x * attackerToTarget.z - attackerAimDirection.z * attackerToTarget.x,
    ...
    private static Vec2 applyVerticalHitAnglePowerTransfer(final float verticalHitAngleScale, final float horizontalPower, final float verticalPower, final Vec3 attackerPosition,
                                                     final Vec3 attackerAimDirection, final Vec3 targetCenteredPosition, final float targetHeight) {
    ...
    private static Vec2 applyVerticalPositionAnglePowerRotation(final float verticalPositionAngleScale, final float horizontalPower, final float verticalPower, final float originalHorizontalPower,
                                                          final float originalVerticalPower, final Vec3 attackerFeetPosition, final Vec3 targetFeetPosition) {
